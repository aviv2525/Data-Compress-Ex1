...
void compress_data( FILE *input, BIT_FILE *output, CODE *codes,unsigned long* counts );
...
void expand_data();
void print_char();
//void MoveCountAndTarget(unsigned long target,unsigned long* counts);
void ZeroCave(int first, unsigned long* counts);
int compare(const void* a, const void* b);
void print_huffman_tree(NODE* nodes, int index, int depth);
#endif  /* __STDC__ */
...
char *argv[];
{
    ...
    compress_data( input, output, codes, counts );
    free( (char *) counts );
    free( (char *) nodes );
    free( (char *) codes );
}
...

{
    ...
    qsort(sortCounts, 256, sizeof(unsigned long), compare);

    unsigned long target = sortCounts[246];
    int targetLocation=0, countDuplicate=0;
   // printf("\nThe Target value: %d\n",target); // the 10 most common tims is
    // Now we find the value location:    
    for (int i = 247; i < 256; i++)//countDuplicate
    {
        if (sortCounts[i] == target)
            countDuplicate++;
    }



    // the target is the 10 beffor the biggest duplucated 
    int flag = 0;
    for (int i = 0; i < 256; i++)
        {

            if (counts[i] == target && flag == 0) 
                {
                    flag = 1;
                    targetLocation = i;
                    continue;
            ...
            if (counts[i] == target && flag != 0) // other times - target get 0 because i need only one doesn't matter who
                {
                if (countDuplicate != 0)
                {
                    countDuplicate--;
    ...
    for (int i = 0; i < 256; i++) // change the counts array to ASCI number 
    {
        if (counts[i] != 0)
            counts[i] = i;
...
void ZeroCave(int first, unsigned long *counts ) {
    int countZero = 0;
    int TheFirst = first;
    if (counts[0] == 0) // Edge Case - in case we have 0 first
        {
            int i = 0;
            while (counts[i] == 0) {
                {
    ...
    if (counts[255] == 0)  // Edge Case - in case we have 0 at the end
       {
            int i = 255;
            while (counts[i] == 0) {
                {
    ...
    {
        countZero++;
        first++;             // count from the first zero till the last
        if (counts[first] != 0) // while we  are in Zero cave this condition should be false and the counter ("countZero") start to count  
                                // now Small letter 'first' is the  no zero
        {
            //for (int i = TheFirst; i < first; i++)
            //    {
                    //int countPutZero = 1;
                    if ((countZero % 2) == 0) // even number of zero
                    {
                        int halfEven = (countZero / 2);
                        int i = 1;
                        while (halfEven)
                        {
                            counts[TheFirst] = counts[TheFirst - i];
                            counts[first - i] = counts[first];
                            i++;
                            TheFirst++;
                            halfEven --;
                    ...
                    if ((countZero % 2) != 0) // Odd number of zero
                    {
                        int halfOdd = (countZero / 2);
                        counts[TheFirst] = counts[TheFirst - 1];
                        TheFirst++;
                        int i = 1;
                        while (halfOdd)
                        {
 ...
 */
void compress_data( input, output, codes, counts)
FILE *input;
BIT_FILE *output;
CODE *codes;
unsigned long* counts;
{           
    ...
    for(int i=0; i<256; i++)
       {
         if(counts[i] == 0)  //real first zero
                ZeroCave(i,counts);
       }
    for(int i=0; i<256; i++)
        {
        codes[i].code = counts[i];
        }
